#!/usr/bin/env bash
# 预下载 OmniVoice 模型到 docker-data/omnivoice-models/hf（一次即可，可断点续传）。
# 下载完成后重启 omnivoice-server 容器即生效，无需重新构建镜像。
set -e
cd "$(dirname "$0")"
# 模型缓存统一放在 docker-data/omnivoice-models/hf（运行期挂载进容器）
mkdir -p ../docker-data/omnivoice-models
...
docker run --rm \
  -v "$HOST_DIR/../docker-data/omnivoice-models/hf:/root/.cache/huggingface" \
...
if [ -z "$(ls -A ../docker-data/omnivoice-models/hf 2>/dev/null)" ]; then
  echo "[失败] docker-data/omnivoice-models/hf 为空：挂载可能未生效（Windows 请在 Git Bash 中运行本脚本）" >&2
  exit 1
fi
echo
